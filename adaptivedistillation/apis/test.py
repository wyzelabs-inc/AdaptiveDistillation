...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        ...
        results, teacher_results = split_teacher_student_results(result, results, teacher_results)
        if show or out_dir:
...
def multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    if torch.is_tensor(result):
        results.extend(teacher_results)
        len_dataset = 2*len(dataset)
    else:
        len_dataset = len(dataset)

    # collect results from all ranks
    if gpu_collect:
        results = collect_results_gpu(results, len_dataset)
    else:
        results = collect_results_cpu(results, len_dataset, tmpdir)
    return results
...
def split_teacher_student_results(result, s_results, t_results):
    if torch.is_tensor(result):
        batch_size = result.shape[1]
        s_results.extend(result[0])
        t_results.extend(result[1])
    else:
        batch_size = len(result)
        s_results.extend(result)
